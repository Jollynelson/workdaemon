...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    // Wire to your real API: POST /api/auth/login
    try {
      ...
      // Demo fallback — any valid email format proceeds
      if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) && password.length >= 8) {
        navigate('/app');
        return;
      }
      setError(res ? ((await res.json()).error ?? 'Invalid credentials.') : 'Use any valid email + 8+ char password to demo.');
    } finally {
      setLoading(false);
    }
  }
