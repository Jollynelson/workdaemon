...
function StepInvite({ data, setData }) {
  ...
  const handleCopy = () => {
    navigator.clipboard?.writeText('https://workdaemon.io/invite/your-workspace');
    setCopied(true);
...
export default function Onboarding() {
  ...
  const getButtonLabel = () => {
    ...
    if (step === 5) return 'Authenticate with GitHub';
    if (step === 6) return 'Authenticate with Slack';
    return 'Continue';
  };
  ...
  const forms = [
    ...
    <StepInvite    key={4} data={data} setData={setData} />,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 32 }}>
            ...
            {isOptional && (
              <button type="button" onClick={advance}
                style={{ background: 'none', border: 'none', fontFamily: 'var(--dmsans)', fontSize: 14, color: 'rgba(232,232,232,0.30)', cursor: 'pointer', padding: '10px 4px', transition: 'color 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.color = 'rgba(232,232,232,0.55)'; }}
                onMouseLeave={e => { e.currentTarget.style.color = 'rgba(232,232,232,0.30)'; }}
              >Skip</button>
            )}
            <button
              type="button"
              ...
              style={{
                padding: '10px 24px',
                background: canAdvance() ? '#4172f5' : 'rgba(255,255,255,0.06)',
                border: 'none',
                borderRadius: 8,
              ...
              onMouseEnter={e => { if (canAdvance()) { e.currentTarget.style.background = '#5281ff'; e.currentTarget.style.boxShadow = '0 4px 16px rgba(65,114,245,0.3)'; } }}
              onMouseLeave={e => { if (canAdvance()) { e.currentTarget.style.background = '#4172f5'; e.currentTarget.style.boxShadow = 'none'; } }}
            >
              {getButtonLabel()}
            </button>
          </div>
