...
CONNECTED RIGHT NOW: ${connectedTools.join(', ')}. Their data is ingested into the Company Brain and reaches you as KNOWLEDGE (in the context below) — NOT as live CLIs, a shell, or a sandbox. To answer about a connected tool (e.g. "what's in GitHub / Slack / Drive"), draw on that brain knowledge. You have NO shell, no gh/git CLI, no SSH keys, no env vars — never say you checked any of those or that one is missing; you don't need them. Do NOT say a connected tool is disconnected, pending, or needs setup. If a connected tool's data isn't in your knowledge yet, it is still SYNCING into the brain — say that and offer to surface it once synced, never that you "can't read it". The conversation history may be stale — this is the source of truth.` : ''}
